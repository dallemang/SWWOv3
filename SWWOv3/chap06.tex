...
SPARQL 1.0, so the basic features are available for both releases \cite{Harris:13:SQL}. 
...
person, as shown in Figure~\ref{fig:ch6.2}. Who were the directors that
...
\begin{query}Actors who worked with James Dean and John Ford\end{query}
\begin{lstlisting}
SELECT ?q3
WHERE { :JamesDean :playedIn ?q1 .
        ?q3 :playedIn ?q1 .
...
\begin{query}What about James Dean?\end{query}
\begin{lstlisting}
SELECT ?property
WHERE { :JamesDean ?property ?value }
\end{lstlisting}
...
\begin{query}What about any actor?\end{query}
...
\begin{query}Find all properties in the data\end{query}
\begin{lstlisting}
SELECT DISTINCT ?property
...
The Linked Movie DataBase (LinkedMDB, \cite{ldow09/HassanzadehC}) is an 
open data source containing
...
\begin{query}Tragic Rebel Actors (last)\end{query}
...
\begin{query}Total sales amounts by year\end{query}
...
\begin{query}Comparative sales year to year\end{query}
...
\begin{query}Full names of Authors\end{query}
