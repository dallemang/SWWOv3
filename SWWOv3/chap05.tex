...
to allow it to grow to global size.   The  Web relies on open standards to specify the components of its
architecture, allowing them to t span different software and hardware, making it implementation-independent and domain-independent. To
support a Web of data, we want to 
the first important constraint was to remain
inside the Web architecture and make as few changes as possible. By
reusing as much of the Web architecture as possible, the Web of data
...
the Web. The Web of data applies the Web architecture to structured data
sharing on a World Wide scale. Meanwhile it maintains its architectural
support for decentralization, extensibility and openness.

In the architecture of the Web of data, what are the changes we
performed to be able to describe resources and publish this data? As
shown in Figure 1 this requires two steps: URLs are generalized to URIs
or IRIs, and HTML is replaced by RDF and its syntaxes.
...
The first step is to support the exchange of data instead of documents.
This is done by adding another representation means to the Web, by using
RDF to represent and exchange data where HTML was used to represent and
exchange documents. The second step is to generalize the notion of
universal locator or addresses (URLs) used to locate Web resources
(pages, images, etc.) to the notion of universal identifiers (URIs) used
to identify and then describe virtually anything.
...
of linked data, i.e. datasets and data pieces in them linked across the
world wide web in the same way Web pages, Web sites and anchored texts
in their content are linked across the World on the Web. Instead of
using HTML to write Web pages including links to other pages, the Web of
data uses RDF to write data descriptions including links to other
descriptions.

Tim Berners-Lee introduced the best practices for linked data in his
...
architectural principles and explaining the thinking behind the
specifications. Among the things we will explain in detail in this
chapter are the three rules that must be followed to weave this Web of
data:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Use HTTP URIs to name everything: Just like URLs were introduced to
  ...
  dereference the name of the things they identify.
\item
  ...
  standard languages and in particular RDF and its syntaxes.
\item
