...
(\url{http://animals.org/cat\#mytsie}).  As we include IRIs in the Web of data, we identify,
on the web, what exists, in any language
(e.g., http://الحيوانات.tn/\textbf{猫}\#mytsie)As  In chapt~er\ref{ch3} we
introduced namespaces and QNames (Qualified names) to simplify the
writing of URIs. A possible QName representation for the identifier of
``mytsie'' could then be, for instance, the QName ``cat:mytsie'' with
