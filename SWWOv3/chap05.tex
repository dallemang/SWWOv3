...
In this way, \emphh{conneg} allows us some flexibilty in the use of HTTP URIs;  we can use 
...
This URL resolves to an HTML page that displys all the data about this
resource.   
...
\begin{itemize}
\item
  \emph{DBPedia} is a central and historical source of data and identifiers in
  ...
  open data.
\item
  \emph{Wikidata} is an initiative of the Wikimedia foundation to crowdsource
...
In this book, we have included a number of examples from the Web such as the
data sources we just mentioned. At the time of this writing, all of these sources are
available, but, of course,  anything can change on the Web at any moment.  We have done our best to select
sources that have already shown some longevity, so that we can have some confidence that they 
will not have disappeared by the time you read these lines.  
...
(inria.fr). The request uses the verb POST (line 1). As we saw in Figure~\ref{fig:ch5.17}, POST to a container creates a new member of that container. The
